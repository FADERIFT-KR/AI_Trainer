...
완료된 반복은 먼저 7개 DTW 거리 meta-feature로 정상 확률을 계산하고, 비정상일 때만
오류별 median/IQR 정규화 거리와 z-margin으로 발뒤꿈치·엉덩이 하방·고관절 오류를
구분합니다. 발뒤꿈치는 준비 자세에서 고정한 2D heel/toe 높이를 보조 근거로 사용하며,
발목 각도·관절 속도·골반 궤적은 자세 유사도 계산에서 계속 제외합니다.

```powershell
python scripts/run_offline_dtw_eval.py --algorithm dtw
```

보고서 `output/dtw_eval/offline_eval_report.json`에는 actor-disjoint validation의
Accuracy, Macro/Weighted Precision·Recall·F1, ROC-AUC와 calibration actor 5-fold
group cross-validation 결과가 저장됩니다. AIHub에는 별도의 UI 카운트다운이 없으므로
오프라인 2D heel 평가는 준비 phase를 baseline으로 사용하는 대체 지표입니다.

캘리브레이션 데이터는 기본적으로 속도 변화(0.8/1.2배), 작은 Y축 회전(±5도),
시간적으로 부드러운 좌표 jitter를 학습 fold 안에서만 적용합니다. 고관절 오류는
증강 대상에서 제외되고 원본만 사용되며, 검증 fold와 최종 validation은 항상 원본
시퀀스로만 평가합니다. 증강 후보의 grouped-CV 목표점수가 무증강보다 최소 0.01
높을 때만 채택하여 미미한 표본 변동으로 운영 calibration이 바뀌는 것을 막습니다.
증강 없는 비교 보고서는 다음처럼 만들 수 있습니다.

```powershell
python scripts/run_offline_dtw_eval.py --algorithm dtw --augmentation none `
  --output output/dtw_eval/offline_eval_report_no_augmentation.json
```
